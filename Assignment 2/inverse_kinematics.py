...
def input_robot_features():
    ...
    return (float(input('Enter a2: ')), float(input('Enter a3: ')),
            float(input('Enter d3: ')), float(input('Enter d4: ')))
...
def inverse_kinematics(T, a2, a3, d3, d4):
    ...
    theta4.append(math.degrees(
        math.atan2((-r13*s1)+(r23*c1), (-r13*c1*c23)-(r23*s1*c23)+(r33*s23))))
    s1 = math.sin(math.radians(theta1[1]))
    c1 = math.cos(math.radians(theta1[1]))
    ...
    theta5.append(math.degrees(
        math.atan2((-r13*(c1*c23*c4+s1*s4)) - (r23*(s1*c23*c4))+(r33*s23*c4),
    ...
    theta6.append(math.degrees(math.atan2(
        ...
        + (r21*((s1*c23*c4-c1*s4)*c5-s1*s23*s5))-(r31*(s23*c4*c5+c23*s5)))))
    s1 = math.sin(math.radians(theta1[1]))
    c1 = math.cos(math.radians(theta1[1]))
    s23 = math.sin(math.radians(theta2[1]+theta3[0]))
    c23 = math.cos(math.radians(theta2[1]+theta3[0]))
    ...
    joint_angles.append(
        (theta1[0], theta2[0], theta3[0],
         theta4[0]+180, -theta5[0], theta6[0]+180))
    joint_angles.append(
        (theta1[1], theta2[1], theta3[0],
         theta4[1]+180, -theta5[1], theta6[1]+180))
    joint_angles.append(
        (theta1[0], theta2[2], theta3[1],
         theta4[2]+180, -theta5[2], theta6[2]+180))
    joint_angles.append(
        (theta1[1], theta2[3], theta3[1],
         theta4[3]+180, -theta5[3], theta6[3]+180))
...
def print_solution(joint_angles):
    ...
    for i in range(8):
        print('theta1: {}, theta2: {}, theta3: {}, \
            theta4: {}, theta5: {}, theta6: {}'.format(
            joint_angles[i][0], joint_angles[i][1], joint_angles[i][2],
            joint_angles[i][3], joint_angles[i][4], joint_angles[i][5]))
        print()
