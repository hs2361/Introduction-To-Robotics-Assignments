...
def input_dh_table(num_joints):
    ...
    for i in range(num_joints):
        parameters = np.array([float(input('alpha{}: '.format(i))),
                               float(input('a{}: '.format(
                                   i))), float(input('d{}: '.format(i+1))),
...
def T(dh_table, i):
    ...
    return np.array([[math.cos(theta_i), -math.sin(theta_i), 0, a_i1],
                     [math.cos(alpha_i1)*math.sin(theta_i),
                      ...
                      -math.sin(alpha_i1), -d_i*math.sin(alpha_i1)],
                     [math.sin(alpha_i1)*math.sin(theta_i),
                      math.sin(alpha_i1) * math.cos(theta_i),
                      math.cos(alpha_i1), d_i*math.cos(alpha_i1)],
                     [0, 0, 0, 1]])
